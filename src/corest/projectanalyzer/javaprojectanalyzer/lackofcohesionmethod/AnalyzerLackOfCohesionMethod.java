...
public class AnalyzerLackOfCohesionMethod implements JavaProjectAnalyzer {
    ...
    public static double lackOfCohesion(String file){
        int accesses = 0;
         ...
         for (Method method : AnalyzerLackOfCohesionMethod.getMethods(file)) {
            for(String attribute : AnalyzerLackOfCohesionMethod.identifyAttributeNames(file)){
    ...
    public Analysis getAnalysis() {
        double average =0.0;
        for (Method method: getMethods(classCode)) {
            average+= method.getNumberOfLines();
        }
        if (getMethods(classCode).isEmpty()) {
            average = 0;
        }
        else {
           average = average /(double) getMethods(classCode).size(); 
        }
        return new Analysis("LackOfCohesion", className, Double.toString(average));
    }

    @Override
    public void setAnalizerParameter(String... analizerParameter) {
        this.className = analizerParameter[0];
        this.classCode = analizerParameter[1];
    }
}